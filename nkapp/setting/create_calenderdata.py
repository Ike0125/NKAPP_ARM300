...
from nkapp.models import TradingCalendar, Tl, engine  # `Tl`は`daily_all_table`を含むモデル
...
def populate_trading_calendar():
    try:
        ...
        existing_dates = session.query(TradingCalendar.tradingdate).distinct().all()
...
#        for i, trading_date in enumerate(new_dates, start=len(existing_dates_set) + 1):
